...
class SecondaryStructure:
    ...
    def __init__(self, data):
        ...
        if not isinstance(data, pd.DataFrame):
            raise AttributeError('must be initialized with instance of pandas DataFrame')
...
def compute_secondary_structure(structure_or_trajectory, executable='stride'):
    """
    Compute the secondary structure for :func:`simlib.Structure` or :func:`simlib.Trajectory`.

    Parameters
    ----------
    structure_or_trajectory : Structure or Trajectory
    ...
    # If not Structure or Trajectory, throw an error
    if not isinstance(structure_or_trajectory, (Structure, Trajectory)):
        raise AttributeError('cannot interpret %s' % structure_or_trajectory)

    # Filter only peptide atoms out of Structure or Trajectory
...
def _compute_secondary_structure(structure, executable='stride'):
    ...
    -------
    SecondaryStructure
        The secondary structure of the Structure
    """
    ...
    # Return
    return secondary_structure.set_index('residue_id').iloc[:, 0]
